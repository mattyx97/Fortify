import { listOrgMembers } from '#server/services/members'
import { getOrgMembership } from '#server/services/organizations'
import { listMembersAbility } from '#shared/utils/abilities/org-member'
import { z } from 'zod'
...
export default defineAuthEventHandler({
  ...
  async handler(event, { input: { params }, identity: { user } }) {
    ...
    await authorize(event, listMembersAbility, { membership })
    return listOrgMembers({ orgId: params.orgId })
  },
})
